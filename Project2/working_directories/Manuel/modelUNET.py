...
def conv2d_block(input_tensor, n_filters, kernel_size=3,dropout=0.2, batchnorm=False):
    # first layer
    x = Conv2D(filters=n_filters, activation='relu', kernel_size=(kernel_size, kernel_size), kernel_initializer="he_normal",
...
def get_unet(input_img, num_classes, n_filters=16, dropout=0.2, batchnorm=False):
    # contracting path
    ...
    # expansive path
    u6 = Conv2DTranspose(n_filters*8, (3, 3),activation='relu', strides=(5, 5), padding='same') (c5)
    u6 = concatenate([u6, c4])
    c6 = conv2d_block(u6, n_filters=n_filters*8, kernel_size=3,dropout=dropout, batchnorm=batchnorm)

    u7 = Conv2DTranspose(n_filters*4, (3, 3),activation='relu', strides=(2, 2), padding='same') (c6)
    u7 = concatenate([u7, c3])
    c7 = conv2d_block(u7, n_filters=n_filters*4, kernel_size=3,dropout=dropout, batchnorm=batchnorm)

    u8 = Conv2DTranspose(n_filters*2, (3, 3),activation='relu', strides=(2, 2), padding='same') (c7)
    u8 = concatenate([u8, c2])
    c8 = conv2d_block(u8, n_filters=n_filters*2, kernel_size=3,dropout=dropout, batchnorm=batchnorm)

    u9 = Conv2DTranspose(n_filters*1, (3, 3),activation='relu', strides=(2, 2), padding='same') (c8)
