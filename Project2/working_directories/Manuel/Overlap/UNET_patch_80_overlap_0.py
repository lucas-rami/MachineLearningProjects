...
sel_imgs_patches_add, sel_gts_patches_add = select_imgs(imgs_patches_add.reshape(1,imgs_patches_add.shape[0],patch_size,patch_size,3), gts_patches_add.reshape(1,gts_patches_add.shape[0],patch_size,patch_size), sel_threshold)
all_imgs_patches = np.append(imgs_patches, sel_imgs_patches_add,axis=0)
all_gts_patches = np.append(gts_patches, sel_gts_patches_add,axis=0)
all_gts_patches = np.expand_dims(all_gts_patches,axis=3)
...
gts_patches_val = np.expand_dims(gts_patches_val,axis=3)
...
model = get_unet(input_img,num_classes = 1,n_filters=16, dropout=0.2, batchnorm=True)

model.compile(optimizer=Adam(), loss="binary_crossentropy", metrics=[f1_score])
model.summary()

checkpointName = 'UNET_patch_80_overlap_0'
...
model_train = model.fit(all_imgs_patches, all_gts_patches, batch_size=batch_size,epochs=epochs,callbacks=callbacks,verbose=1,validation_data=(imgs_patches_val, gts_patches_val))
