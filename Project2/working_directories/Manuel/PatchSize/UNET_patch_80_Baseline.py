...
from modelUNET import get_unet_80_bl,f1_score
...
model = get_unet_80_bl(input_img,num_classes=num_classes,n_filters=16, dropout=0.5, batchnorm=True)

model.compile(optimizer=Adam(), loss="binary_crossentropy", metrics=[f1_score, 'accuracy'])
model.summary()

checkpointName = 'UNET_patch_80_bl'
