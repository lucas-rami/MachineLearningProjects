...
def get_unet_2_100(input_img, num_classes = 2, n_filters=16, dropout=0.2, batchnorm=False):
    # contracting path
    ...
    # expansive path
    u4 = Conv2DTranspose(n_filters*2, (3, 3),activation='selu', strides=(2, 2), padding='same') (c3)
    u4 = concatenate([u4, c2])
    c4 = conv2d_block(u4, n_filters=n_filters*2, kernel_size=3,dropout=dropout, batchnorm=batchnorm)

    u5 = Conv2DTranspose(n_filters*1, (3, 3),activation='selu', strides=(2, 2), padding='same') (c4)
    u5 = concatenate([u5, c1], axis=3)
    c5 = conv2d_block(u5, n_filters=n_filters*1, kernel_size=3,dropout=dropout, batchnorm=batchnorm)
