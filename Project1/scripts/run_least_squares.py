...
for degree in degrees: # For each degree...
    ...
    for k in k_cross_val: # For each k...
        ...
        print("Trying (degree, k) = (" + str(degree) + ", " + str(k) + ")")

        # Use ridge_regression to compute our model
        weights, pred_score = k_fold_cross_validation(y_train, processed_tx_train, k, imp.least_squares, [])
...
print("Best parameters are (degree, lambda, k) = (" + str(best_degree) + ", " + str(best_k) + ")")
...
filename = program_path + '/results/run_least_squares.csv'
helper.create_csv_submission(ids, y_pred, filename)

# Best score on training data is 0.8156320000000001
# Best parameters are (degree, lambda, k) = (10, 5)
